...
module.exports.validate_inner_services = function(req,res,next){
    if (!config.status.mongodb_alive || !config.status.kafka_alive){
        res.status(500).json({server:"scarlett", http_status:500, status: {alive: false, inner_services: config.status}});
    } else {
        next();
    }
}
